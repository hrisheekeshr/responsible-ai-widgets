...
def compute_json_error_tree(model, dataset, true_y, features, filters,
    ...
    is_pandas = isinstance(dataset, pd.DataFrame)
    if is_pandas:
        true_y = true_y.to_numpy()
    else:
        input_data = input_data.to_numpy()
    diff = model.predict(input_data) != true_y
    ...
    if categorical_features:
        ...
        for idx, c_i in enumerate(categorical_indexes):
            input_data[:, c_i] = string_ind_data[row_index, idx]
    ...
    if categorical_features:
        for c_index, feature in enumerate(categorical_features):
            try:
                index_sub = dataset_sub_names.index(feature)
            except ValueError:
                continue
            cat_ind_reindexed.append(index_sub)
            categories_reindexed.append(categories[c_index])
